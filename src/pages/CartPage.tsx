...
import { Minus, Plus, Trash2, ArrowLeft, ShoppingBag, Check, Loader2, ShieldCheck } from 'lucide-react'
...
const supabase = createClient(supabaseUrl, supabaseAnonKey);
      if (!user) {
        // Use mock data for demo if no user is logged in
        setCartItems([
          {
            id: '1',
            supplement_id: '1',
      ...
      const { data, error } = await supabase
        .from('cart_items')
        .select('*, supplement:supplement_id(id, name, price_aed, image_url, brand)')
        .eq('user_id', user.id);
      ...
      if (data && data.length > 0) {
        ...
        // Try supplements table as fallback
        ...
        if (!alternativeError && alternativeData) {
          setCartItems(alternativeData as CartItemWithDetails[]);
        } else {
  ...
  const subtotal = cartItems.reduce((sum, item) => {
    const price = item.supplement?.price_aed || 0;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 p-4 rounded-lg">
            ...
            <Button onClick={loadCartItems} className="mt-4">Try Again</Button>
          </div>
        ) : (
        cartItems.length === 0 ? (
                    ...
                    {cartItems.map((item) => (
                      <div key={item.id} className="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg">
                        ...
                        <div className="flex-1 ml-4">
                          ...
                          <p className="text-gray-600 mt-1">{item.supplement?.price_aed?.toFixed(2) || 0} AED</p>
                        </div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{shipping.toFixed(2)} AED</span>
                  </div>
